...
PaddleOCRの結果はUnicode正規化後に空白だけを除いて比較し、隣接する完全一致字幕のみ統合します。
数字や文字が違う結果は類似度で統合しません。
...
300フレームから68字幕区間を生成し、OCRと翻訳の処理エラーは0件でした。
保存JSONは字幕画像を含むため、この条件で約14.6MBでした。

手動2行分割は、別の開発用動画の369.5〜371.0秒で上段と下段の順序保持を確認しました。
認識文には行端のノイズが残っています。

15分程度の動画ファイルの選択と再生に上限はありませんが、15分全体のOCRと翻訳は未検証です。
固定200ms抽出は短い字幕を見逃す場合があり、区間境界に最大200ms程度の誤差が生じます。
完全一致だけを統合するため、同じ字幕でも1文字のOCR揺れによって複数区間へ分かれます。
